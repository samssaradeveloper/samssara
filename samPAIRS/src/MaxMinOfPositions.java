	...
	public void AdjustMaxMinOfTrades(DataMaster DataMasterObj) {
		try {
			...
			for(int i=0;i<OpenPosition.size();i++) {
				...
				if(LiveRatio > Double.valueOf(PropertyList[0]) && LiveRatio != 0) {
					PropertyList[0] = String.valueOf(LiveRatio);
					ChangesMade = 1;
				}
				
				//if the max or min values are changed then access them here
				if(LiveRatio < Double.valueOf(PropertyList[1]) && LiveRatio != 0) {
					PropertyList[1] = String.valueOf(LiveRatio);
					ChangesMade = 1;
				}				
