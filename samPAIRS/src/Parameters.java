...
public class Parameters {
	...
	public long StopTradingBeforeModelStopTime = 0;
...
//		UITradesPath = RootDirectory + "UITrades.txt";
	...
	public String ModelStopOnDayOfWeek = "7-1000";
	public int WriteToBBGFile = 1;
	public int LimitOrderExecution = 0;
	public double CricticalZValue = -2;
	public double PrevCricticalZValue = 0;	
	public double CriticalVRT = 100;
	public double CriticalOU = 100;
	public double ExecutionCutOff = 0.03;
	public double TimeStopLoss = 0;
	public int OptionsTradingActive = 0;
	public double OptionsOrderValue = 5000;
	public double OrderValue = 300000;
	public double VolumeCutOff = 0;
	int FTPUpdate = 0;
	int DBUpdate = 1;
	int RunMidMinute = 0; 
	...
	public void SetParameterValues(ArrayList ParamList) {
		try {
			...
			for(int i=0;i<ParamList.size();i++) {
				...
				if(Field.equalsIgnoreCase("StopTradingBeforeModelStopTime")) {
					StopTradingBeforeModelStopTime = Long.parseLong(Value);
				}				
				...
				if(Field.equalsIgnoreCase("CricticalZValue")) {
					CricticalZValue = Double.parseDouble(Value);
				}
				if(Field.equalsIgnoreCase("PrevCricticalZValue")) {
					PrevCricticalZValue = Double.parseDouble(Value);
				}				
				if(Field.equalsIgnoreCase("CriticalVRT")) {
					CriticalVRT = Double.parseDouble(Value);
				}
				if(Field.equalsIgnoreCase("CriticalOU")) {
					CriticalOU = Double.parseDouble(Value);
				}												
				if(Field.equalsIgnoreCase("TimeStopLoss")) {
					TimeStopLoss = Double.parseDouble(Value);
				}
				if(Field.equalsIgnoreCase("OptionsTradingActive")) {
					OptionsTradingActive = Integer.parseInt(Value);
				}			
				if(Field.equalsIgnoreCase("FTPUpdate")) {
					FTPUpdate = Integer.parseInt(Value);
				}			
				if(Field.equalsIgnoreCase("DBUpdate")) {
					DBUpdate = Integer.parseInt(Value);
				}			
				if(Field.equalsIgnoreCase("OptionsOrderValue")) {
					OptionsOrderValue = Double.parseDouble(Value);
				}
				if(Field.equalsIgnoreCase("OrderValue")) {
					OrderValue = Double.parseDouble(Value);
				}
				if(Field.equalsIgnoreCase("VolumeCutOff")) {
					VolumeCutOff = Double.parseDouble(Value);
				}
				if(Field.equalsIgnoreCase("RunMidMinute")) {
					RunMidMinute = Integer.parseInt(Value);
				}			
			}	
		}
		catch(Exception e) {
	...
	public String getDBUserName(DataMaster DataMasterObj){
		String DBUser = "";
	    String Exchange = DataMasterObj.Param.ClientUniqueCode.split("_")[1];
	    DBUser = "samssara_"+Exchange;
	    return DBUser;
	}
