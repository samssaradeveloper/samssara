...
public class ExpiryRollover {
	...
	public void CheckExpiryDatesOfPositions(DataMaster DataMasterObj) {
		try {
			...
			for(int i=0;i<DataMasterObj.Futures.length;i++) {
				...
				if(ExpiryRisk == 1) {
					String ExpiringStockCode = DataMasterObj.FuturesCode[i];			
					...
					for(int j=0;j<OpenPosition.size();j++) {						
						...
						if(ExpiringStockCode.equals(StockCode1) || ExpiringStockCode.equals(StockCode2)) {
							...
							if(ExpiryDataForPosition.trim().equals(UtilObj.ConvertToYYYYMMDD(DataMasterObj.ExpiryDatesCurrentExpiry[i]))) {
								//Getting the order string to send the rollover trades to the markets
								String[] OrderString = GetRolloverOrderStrings(DataMasterObj, OpenPositionData);
								//Getting the trade string to write the rollover trades in the trades,txt file
								String[] TradeString = GetRolloverTradeStrings(DataMasterObj, OrderString);
								
								if((OrderString[0] != "" && OrderString[1] != "") || (OrderString[2] != "" && OrderString[3] != "")){
									if(UtilObj.CheckConnection(DataMasterObj) == 0) {
										if(DataMasterObj.Param.AutoMode.equals("ON")) {
											...
											if (OrderString[2] != "" && OrderString[3] != ""){
												DataMasterObj.os.write(OrderString[2]);
												DataMasterObj.os.flush();
												
												DataMasterObj.os.write(OrderString[3]);
												DataMasterObj.os.flush();
											}
										...
										UtilObj.WriteToFile(MarketFile, OrderString[2] + "... Rollover", true);
										UtilObj.WriteToFile(MarketFile, OrderString[3] + "... Rollover", true);	
	...
	public static String[] GetRolloverOrderStrings(DataMaster DataMasterObj, String[] OpenPositionData) {
		String[] RollOverStrings = new String[4];
		for(int i=0;i<RollOverStrings.length;i++) {
			RollOverStrings[0]="";RollOverStrings[1]="";RollOverStrings[2]="";RollOverStrings[3]="";
		}
		try {
			Utils UtilObj = new Utils();
			String StockCode1 = OpenPositionData[1];
			...
			if (!StockCode1.equals(DataMasterObj.FuturesCode[0])){
				RollOverStrings[0] = "SELL"+"|"+DataMasterObj.ExchangeCode[StockIndex1]+"|"+DataMasterObj.InstrumentCode[StockIndex1]+"|"+StockName1+"|"+"1"+"|"+Lot1+"|"+"0"+"|"+DataMasterObj.Param.ClientCode+EndLine;
				RollOverStrings[1] = "BUY"+"|"+DataMasterObj.ExchangeCode[StockIndex1]+"|"+DataMasterObj.InstrumentCode[StockIndex1]+"|"+StockName1+"|"+"2"+"|"+Lot1+"|"+"0"+"|"+DataMasterObj.Param.ClientCode+EndLine;				
			}
			if (!StockCode2.equals(DataMasterObj.FuturesCode[0])){
				RollOverStrings[2] = "BUY"+"|"+DataMasterObj.ExchangeCode[StockIndex2]+"|"+DataMasterObj.InstrumentCode[StockIndex2]+"|"+StockName2+"|"+"1"+"|"+Lot2+"|"+"0"+"|"+DataMasterObj.Param.ClientCode+EndLine;
				RollOverStrings[3] = "SELL"+"|"+DataMasterObj.ExchangeCode[StockIndex2]+"|"+DataMasterObj.InstrumentCode[StockIndex2]+"|"+StockName2+"|"+"2"+"|"+Lot2+"|"+"0"+"|"+DataMasterObj.Param.ClientCode+EndLine;
			}
		}
		catch(Exception e) {
			e.printStackTrace();
